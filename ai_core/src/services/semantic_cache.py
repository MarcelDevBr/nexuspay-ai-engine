...
class SemanticCacheService:
    def __init__(self):
        try:
            ...
            logger.warning(f"Não foi possível conectar ao Redis: {e}. Semantic Cache desativado temporariamente.")
    ...
    def get(self, query_embedding: list[float], threshold: float = 0.92) -> Optional[Tuple[str, float]]:
        """
        Busca no Semantic Cache se existe uma resposta pré-calculada com similaridade de cosseno >= threshold
        """
    ...
    def set(self, query_text: str, query_embedding: list[float], response: str, ttl_seconds: int = 3600):
        """
        Armazena uma pergunta, seu vetor de embedding e a resposta gerada
        """
        if not self.redis_client:
            return
        ...
        try:
            ...
            logger.info(f"Gravado no Semantic Cache com sucesso: {cache_id}")
        except Exception as e:
